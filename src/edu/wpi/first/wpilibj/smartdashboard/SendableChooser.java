...
public class SendableChooser<V> implements Sendable {
  ...
  public void initTable(ITable table) {
    m_table = table;
    if (table != null) {
    	Set<String> thing = m_map.keySet();
    	String[] thingArray;
    	thingArray = thing.toArray(new String[0]);
    	boolean reordered = true;
    	while(reordered) {
    		reordered = false;
    		for(int i=1; i<thingArray.length; i++) {
    			String temp;
    			if(thingArray[i-1].compareTo(thingArray[i]) > 0) {
    				temp = thingArray[i-1];
    				thingArray[i-1] = thingArray[i];
    				thingArray[i] = temp;
    				reordered = true;
    			}
    		}
      ...
      table.putStringArray(OPTIONS, thingArray); //m_map.keySet().toArray(new String[0]));
