...
//    	this(distance, Math.abs((distance*120.0)/(TmSsDriveTrain.getMaxFtPerSecLowGear()*pKy/2)));
    	this(distance, distance, DEFAULT_TIMEOUT, FeedbackTypesE.DEFAULT, FeedbackResetsE.DEFAULT);
    }
    public TmACmdDriveStraightEndWithGyro(double distance, double timeout) {
    	this(distance, distance, timeout, FeedbackTypesE.DEFAULT, FeedbackResetsE.DEFAULT);
    }
    public TmACmdDriveStraightEndWithGyro(double distance, FeedbackTypesE fbType) {
    	this(distance, distance, DEFAULT_TIMEOUT, fbType, FeedbackResetsE.DEFAULT);
    }
    public TmACmdDriveStraightEndWithGyro(double distance, double timeout, FeedbackTypesE fbType) {
    	this(distance, distance, timeout, fbType, FeedbackResetsE.DEFAULT);
    }
    public TmACmdDriveStraightEndWithGyro(double distanceDfltAndRed, double distanceBlue, double timeout, FeedbackTypesE fbType, FeedbackResetsE fbReset) {
    	ssDrive = TmSsDriveTrain.getInstance();
    	drvSixMotors = ssDrive.getChassisInstance();
    	drvEncMgmt = ssDrive.getDriveEncoderMgmtInstance();
    	thisCmdName = Tt.extractClassName(this) + "(" + distanceDfltAndRed + ", " + distanceBlue + ", " + timeout + ")";
        m_fbType = fbType;
        m_fbReset = fbReset;
...
//        P.println(-1, thisCmdName + ": INITIALIZING: " + inputDescr);
        ...
        if(m_fbReset.isResetGyro()) { ssDrive.resetGyro(); } //cb_robotDrive.resetGyro(); }
...
//    		double theta;
    		...
    		double dy;
    		rdg = getEncoderRdg();
    		theta = Math.toRadians(ssDrive.getAngle());
