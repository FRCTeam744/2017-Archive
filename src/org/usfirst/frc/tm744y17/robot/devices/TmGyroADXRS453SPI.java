 ...
 *
 */
public class TmGyroADXRS453SPI extends SensorBase implements TmMiscSdKeys, LiveWindowSendable {
	...
	private RoNamedIoE m_gyroNamedIoDef;
...
//	private int m_tfp;
	private boolean m_foundSpiGyro;
	...
	public void postToSdAngle() {
		if(m_sdKeyAngle != null) {
			TmPostToSd.dbgPutNumber(m_sdKeyAngle, getAngle());
		}
	}
	public void postToSdRate() {
		if(m_sdKeyRate != null) {
			TmPostToSd.dbgPutNumber(m_sdKeyRate, getRate());
		}
	}
	public void postToSdTemperature() {
		if(m_sdKeyTemperature != null) {
			TmPostToSd.dbgPutNumber(m_sdKeyTemperature, getTemperature());
		}
	}
	
	public TmGyroADXRS453SPI(RoNamedIoE driveTrainGyro) {
		this(driveTrainGyro, null, null, null);
	}	
	public TmGyroADXRS453SPI(RoNamedIoE driveTrainGyro,
			SdKeysE sdKeyAngle, SdKeysE sdKeyRate, SdKeysE sdKeyTemp) {
		m_sdKeyAngle = sdKeyAngle;
		m_sdKeyRate = sdKeyRate;
		m_sdKeyTemperature = sdKeyTemp;

		m_gyroNamedIoDef = driveTrainGyro;
		m_gyroSpiPort = driveTrainGyro.getNamedConnDef().getSpiPort();
		...
		m_classNameId= "[" + Tt.extractClassName(this) + "]";
		m_gyroErrorCount = 0;
		init();
...
//		m_tfp = TmHdwrRo.isRunningOnSwTestFixture() ? 0 : -1;
...
//		UsageReporting.report(tResourceType.kResourceType_ADXL345, tInstances.kADXL345_SPI);
	...
	private class TimestampedRate {
		...
		public double rate;      //as returned by calcAngularDegreesPerSecond(CmdAndResponse cr);
...
//		public void set(double calculatedRate, double time) {
//			rate = calculatedRate;
//			timestamp = time;
//		}
	}
	
	private int errMsgCnt = 100;
	/**
	...
	public double getRate() {
		...
		if(isReady()) { 
			...
			if(cr.respType != RespTypeE.SENSOR_DATA) {
				P.println(errMsgCnt, dbgMsgPrefix + ": unexpected response type " + cr.respType.toString());
				errCnt++;
			}
			else if(SensorDataRespBitMaskE.getStatus(cr.resp) != SensorDataResponseStatusE.VALID_DATA.value) {
				P.println(errMsgCnt, dbgMsgPrefix +  ": invalid data.");					
				errCnt++;
			}
			else if(SensorDataRespBitMaskE.getFaults(cr.resp) != 0) {
				P.println(errMsgCnt, dbgMsgPrefix +  ": fault bits are on");					
				errCnt++;
			}
			else {
			...
			SmartDashboard.putNumber(TmMiscSdKeys.SdKeysE.KEY_DRIVE_GYRO_ERROR_CNT.getKey(), m_gyroErrorCount);
			if(errMsgCnt>0) {
				errMsgCnt -= errCnt;
				if(errMsgCnt<0) {errMsgCnt = 0; }
			}
	//		P.println(-1, cr.toStringFull());
		}
		return ans;
	}
	...
	private void updateAngle() {
		...
		if(m_tsRate.rate != 0.0) {
			if(dbgMsgCnt-- > 0) {
				P.println("Gyro detected motion: current degPerSec=" + m_tsRate.rate + ", elapsedTime=" + elapsedTime + ", angle=" + m_angle);
			}
		}
	}
//	private void updateAngle(double curRate, double prevRate, double elapsedTime) {
//		m_angle += (((curRate + prevRate) / 2) * elapsedTime); // /INCR_CNT_PER_DEGREE_PER_SEC;
//	}
	...
	public double getTemperature() {
		...
		if(cr.respType != RespTypeE.READ) {
			P.println(((cr.respType == RespTypeE.INVALID_DUE_TO_P1_PARITY_ERRORS) ? -1 : -1), 
	...
	private double calcAngularDegreesPerSecond(CmdAndResponse cr, double defaultRate) {
		...
		if(cr.respType == RespTypeE.SENSOR_DATA) {
			...
			errCnt++;
			P.println(-1, m_classNameId + "unexpected response type " + cr.respType.toString() + " in calcAngularDegreesPerSecond()");
		}
...
//		boolean gotExpectedData;
		...
		case CMD_FORCE_FAULTS:
			...
			if(crCmd1DummyResp.resp != VERY_FIRST_RESPONSE_FROM_DEVICE) {
				...
				P.println(-1, m_classNameId + "Why didn't we get the response we expected?!? in state " + m_curSte.toString());
			}
			P.println(-1, crCmd1DummyResp.toStringFull());
			m_smTimer.reset();
		...
		case CMD_CLEAR_FAULTS:
			...
			P.println(-1, crCmd2Resp1.toStringFull());
			m_smTimer.reset();
		...
		case CMD_CLEAR_FAULT_BITS_A:
			...
			errCnt = 0;
			if(cr.respType != RespTypeE.SENSOR_DATA) {
				P.println(-1, m_classNameId + "ste " + m_curSte.toString() + ": unexpected response type " + cr.respType.toString());
				errCnt++;
			}
			else if(SensorDataRespBitMaskE.getStatus(cr.resp) != SensorDataResponseStatusE.SELF_TEST_DATA.value) {
				m_foundSpiGyro = true;
				P.println(-1, m_classNameId + "ste " + m_curSte.toString() + ": unexpected status (ST) bits.");					
				errCnt++;
			}
...
//			else if(SensorDataRespBitMaskE.isAllFaultsOn(cr.resp)) {  //TODO why didn't this work??!
				...
				m_foundSpiGyro = true;
//				P.println(-1, m_classNameId + "ste " + m_curSte.toString() + ": not all fault bits are on" +
//			                          " (" + Tt.numToHexString(SensorDataRespBitMaskE.getFaults(cr.resp), 2) +
//			                          " not the expected " + 
//			                          Tt.numToHexString((SensorDataRespBitMaskE.FAULTS.bitMask >>> SensorDataRespBitMaskE.FAULTS.shiftRightCnt), 2) + ")");					
				P.println(-1, m_classNameId + "ste " + m_curSte.toString() + ": not all fault bits are on" +
...
//                        " (" + Tt.numToHexString(SensorDataRespBitMaskE.getFaults(cr.resp), 2) +
//                        " not the expected " + 
//                        Tt.numToHexString(SensorDataRespBitMaskE.FAULTS.bitMask, 2) + ")");					
				P.println(-1, m_classNameId + "ste " + m_curSte.toString() + ": not all fault bits are on" +
                        " (" + String.format("0x%02X", SensorDataRespBitMaskE.getFaults(cr.resp)) +
                        " not the expected " + 
...
//				bitMask = 0x0c0000fe;
//				expectedData = 0x080000fe;
//				gotExpectedData = sendCmdReadSensorData(expectedData, bitMask); 
//				//we don't really care about the data back yet.... (?)
//			m_smTimer.reset();
			m_curSte = StartUpSeqSteMachStatesE.CMD_READ_PART_ID;
			break;
		case CMD_READ_PART_ID:
			...
			if(cr.respType != RespTypeE.SENSOR_DATA) {
				...
				m_foundSpiGyro = true;
				P.println(-1, m_classNameId + "ste " + m_curSte.toString() + ": still have fault bits on");					
				errCnt++;
			}
			else {
				m_foundSpiGyro = true;
		...
		case CMD_READ_TEMP:
			...
			if(cr.respType != RespTypeE.READ) {
				...
				m_foundSpiGyro = true;
//				m_lastRateReadRaw = ReadWriteRespBitMaskE.getData(cr.resp);
			...
			P.println(-1, crCmd10Resp9.toStringFull());
//			P.println(-1, "SPI Gyro partId=" + Tt.numToHexString(m_devPartId, 4) + 
...
//					" rate=" + calcAngularDegreesPerSecond(crCmd9Resp8, 0 /*default rate*/) );
			P.println(-1, "SPI Gyro partId=" + String.format("0x%04X", m_devPartId) + 
					" S/N=" + m_devSerialNbr + " temperature=" + calcDegreesCelcius(crCmd10Resp9.resp) +
					" rate=" + calcAngularDegreesPerSecond(crCmd9Resp8, 0 /*default rate*/) );
			break;
...
//			m_angleTimestamp = 0;
			if(m_foundSpiGyro) {
				...
				P.println(-1, "Never got a valid response to a command. Assume gyro not installed");
...
//			return ((resp & FAULTS.bitMask) /*>>> FAULTS.shiftRightCnt*/); //unsigned right shift
	...
	public class CmdAndResponse {
		...
		public String toStringFull() {
			String ans = "";
//			ans += "cmd=" + Tt.numToHexString(cmd, 8) + " (" + cmdType.toString() + ")";
...
//						" resp=" + Tt.numToHexString(resp, 8) + " (" + respType.toString() + ")";
			ans += " prevCmdType=" + prevCmdType.toString() + 
...
//				ans += " respSeqNbr=" + respSeqNbr + " data=" + Tt.numToHexString(SensorDataRespBitMaskE.getData(resp), 4);
...
//				ans += " data=" + Tt.numToHexString(ReadWriteRespBitMaskE.getData(resp), 4);
	...
	private void bldReadCommand(CmdAndResponse cr, RegisterE regToRead) {
		...
		if((regToRead.regAddr & 0x01) != 0) {
			...
			P.println(-1, m_classNameId + "should only be reading from even-numbered registers, not from " + regToRead.regAddr); 
		}
